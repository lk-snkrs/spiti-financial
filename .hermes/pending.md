...
- [x] heartbeat-state.json — daily_report marcado como FAILED (corrigido do false positive OK)
...
| lk_morning_briefing | OK | 08:21 today |
| hermes_heartbeat | OK | 08:03 today |
...
### LK Cockpit Dashboard
- **Repo**: `/root/lk-cockpit` (Next.js + PostgreSQL on Railway)
- **Dashboard URL**: needs Railway deployment
- **DB Seed**: Updated `prisma/seed.ts` with real cron data (14 jobs)
- **Local Fallback**: `/root/.hermes/hermes-agent/lk_cockpit_dashboard.html` — standalone HTML dashboard from cron_status.json
- **Status**: Build succeeds, Railway CLI not authenticated — needs manual deploy
